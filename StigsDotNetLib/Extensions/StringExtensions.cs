...
namespace StigsDotNetLib.Extensions {
	public static class StringExtensions
	{
		...
		public static T FromJson<T>(this string @this, JsonSerializerSettings settings = null)
		{
			settings = settings ?? new JsonSerializerSettings { TypeNameHandling = TypeNameHandling.Auto };
			return JsonConvert.DeserializeObject<T>(@this, settings);
		}

		public static async Task<T> LoadAsJsonFile<T>(this string @this, JsonSerializerSettings settings = null)
		{
			var json = await File.ReadAllTextAsync(@this);
			return json.FromJson<T>(settings);
		}

		public static async Task<T> LoadAsJsonFileOrDefault<T>(this string @this, JsonSerializerSettings settings = null)
		{
			if (!File.Exists(@this)) return default(T);
			var json = await File.ReadAllTextAsync(@this);
			return json.FromJson<T>(settings);
		}
		public static T LoadAsJsonFileOrDefaultSync<T>(this string @this, JsonSerializerSettings settings = null)
		{
			if (!File.Exists(@this)) return default(T);
			var json = File.ReadAllText(@this);
			return json.FromJson<T>(settings);
		}

		public static bool TryDeserializeFromJsonFile<T>(this string @this, out T result, JsonSerializerSettings settings = null)
		{
			result = default(T);
			if (!File.Exists(@this)) return false;
			result = FromJson<T>(File.ReadAllText(@this));
			return true;
		}
